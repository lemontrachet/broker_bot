...
class Price_Predictor():
    ...
    def predict(self):
        ...
        prediction = float(self.p.predict(X.reshape(1, -1))[0])
    ...
    def fetch_data(self):
        ...
        try:
            ...
            comparitor = Share(share_getter.get_stock_exchange())
        except:
        ...
        try:
            ...
            df['comparitor'] = pd.to_numeric(df['comparitor'])
            return df
        except Exception:
            return df
    ...
    def get_metrics(self, date):
        ...
        returns = returns.resample('M', how='last')
    ...
    def get_price(date, df, market=False):
        ...
        if market == True:
            try:
                return df['comparitor'][df['Date'] == date].values[0]
            except IndexError:
                return 0
        try:
            return df['Adj_Close'][df['Date'] == date].values[0]
        except IndexError:
            return 0
